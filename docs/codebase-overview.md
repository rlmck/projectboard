...
| `app/board-fallback.jpg` | Bundled **fallback** board image (`ProjectBoard.png` as a 260 KB JPEG, same pixels and framing; the live image is in Supabase Storage). Precached; only fetched when there's no live board (offline, or `board_config` failed), since `pb-v101` | yes |
| `app/ProjectBoard.png` | The 2.1 MB source of `board-fallback.jpg`, read by `register_holds.py`/`register_shapes.py`. Still deployed (its URL keeps working) but neither referenced nor precached | yes |
| `app/hold_map.json` | Fallback hold → `{x,y}` % map on the bundled drawing (188 holds; lacks `hold218`, see §13) | yes |
...
|---|---|---|---|
| Board image | `board_config.image_path` → Storage URL `…/board/<path>?v=<updated_at>` | `board-fallback.jpg` (a drawing, different framing) | `BOARD_IMG` |
