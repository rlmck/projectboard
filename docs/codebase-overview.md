...
- **Framing:** it opens framed on the **route** (`fsRouteBox()`: the problem's holds, mirrored if the mirrored view is on, or the circuit's sequence, from `HOLD_MAP`), zoomed to fill the space between the close button and the bar (at most 3×), gliding there from the board's place on the page. Create views open on the whole board.
- **Gestures** (pointer events on the board and backdrop, `touch-action: none`): one finger pans, two pinch about their midpoint (1–4×), the mouse wheel zooms about the cursor. On the read-only detail views a tap hides/shows the controls (`body.fs-ui-hidden`), a double-tap switches route ↔ whole board, and a quick sideways swipe steps along the deck when the board isn't wider than the screen. On create views taps still set holds; after any drag or pinch the next click is swallowed so it never sets one. The page's own detail swipe (`wireDetailSwipe`) is off while the viewer is open.
