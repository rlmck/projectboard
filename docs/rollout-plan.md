...
- **What shipped / deviations from the spec below:**
  ...
  - **Follow-up fix (found checking staging after B4 was pushed):** Cloudflare 307s *every* `.html` URL to its extensionless form (`/privacy.html` → `/privacy`), not just `index.html`. So B4's precache of `./privacy.html` stored a redirect, and offline Privacy failed. `sw.js` now precaches through `precache()`, which re-wraps any redirected response as a plain copy of its body. The offline fallback for an extensionless page (`/privacy`) also tries its `.html` copy. The local mock server copies Cloudflare's full HTML handling (verified against staging), and tests pass for a fresh install and for upgrades from **production `pb-v71`** and **staging `pb-v73`**: offline `/`, `/privacy.html` and `/privacy` all work, and no cached entry is a redirect. `CACHE` stays `pb-v73`, because that version never reached production and the new worker's install overwrites the bad entry on staging phones.
