...
*Last updated: 12 June 2026 — create-a-problem finish zone is now the **top 25% of the board** (positional, from the live hold-map y-span) instead of the fixed "12 highest holds": top-zone holds cycle blue → red → off while no finish is set, then blue → off once one is (no finish-stealing); no starts allowed up top; top holds may be intermediates (top traverse + one finish). SW at `pb-v23`. Earlier this build: phone-native board recalibration — admins upload a new board image to Supabase Storage + Save hold positions to `board_config` (live for everyone, no git); `#calibrate` gains Anchor/Nudge/Add modes; detail board shows true aspect; `hold218`/`hold243` were missing from the map (place via Add). DB scripts go up to `db/10` (new `board_config` table + `board` Storage bucket — run before Save works). Next: full problem editing / mirror mode.*
