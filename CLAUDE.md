...
**Done (this session):**
- **Admin hub (`#admin`)** — a dedicated admins-only view, reached from an **"Admin tools"** button on the profile page (the old "Recalibrate board" button now points here). Same route-guard pattern as `#calibrate` (bounces non-admins once auth is known). Holds two things: a **Recalibrate board** card (links to the unchanged `#calibrate`) and a **Users** section. The contextual per-problem admin buttons (delete / edit-grade) stay inline on the detail header — they belong to a specific problem, not the hub.
- **View & delete users** — the Users section lists every account (username, email, join date, route count, Admin badge). The anon key can't read other users' emails or delete an `auth.users` account, so both go through **SECURITY DEFINER RPCs** gated on `is_admin()`: `admin_list_users()` and `admin_delete_user(target uuid)`. The bin button is **hidden for yourself and for other admins** (UI rail); the RPC **also** refuses self-deletion and admin-deletion (demote in the dashboard first). On delete, the user's account/profile/ticks/likes cascade away but **their problems are kept** — `problems.setter_id` is now `ON DELETE SET NULL`, so routes survive and fall back to the text setter snapshot. Needs **`db/12`** (FK behaviour + the two RPCs); the UI shows a clear "run db/12" message if the RPCs aren't deployed yet. SW at `pb-v26`.
